           ...
           (add-after 'build 'install
             (lambda* (#:key outputs #:allow-other-keys)
               (let* ((out   (assoc-ref outputs "out"))
                 ...
                 (install-file (string-append lib "/ant-junit4.jar") share)
                 (delete-file-recursively bin)
                 (delete-file-recursively lib)
                 #t)))))))
    (inputs
    ...
    (arguments
     `(#:jar-name "java-aqute-libg.jar"
       ...
       #:make-flags (list (string-append "-Dant.build.javac.source=" "1.7")
                          (string-append "-Dant.build.javac.target=" "1.7")); upstream is currently this to 1.6, please check on updates if this is still needed.
