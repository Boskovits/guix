  ...
  (package
    ...
    (arguments
     (substitute-keyword-arguments (package-arguments repro-gcc-wrapped-4.7)
       ((#:phases original-phases)
        `(modify-phases ,original-phases
           (add-after 'install 'fixwrappers
             (lambda* (#:key outputs #:allow-other-keys)
               (let ((out (assoc-ref %outputs "out")))
                 ;;to use this package the environment variable GUIX_GCC_4_7_INSTALL_DIRECTORY must be set to the store path
                 (substitute* (string-append out "/libexec/gcc/x86_64-unknown-linux-gnu/4.7.4/liblto_plugin.la")
                   (("libdir=.*$")
                    "libdir=$GUIX_GCC_4_7_INSTALL_DIRECTORY'/libexec/gcc/x86_64-unknown-linux-gnu/4.7.4'"))
                 (substitute* (string-append out "/lib/libgomp.la")
                   (("libdir=.*$")
                    "libdir=$GUIX_GCC_4_7_INSTALL_DIRECTORY'/lib'"))
                 (substitute* (string-append out "/lib/libitm.la")
                   (("libdir=.*$")
                    "libdir=$GUIX_GCC_4_7_INSTALL_DIRECTORY'/lib'"))
                 (substitute* (string-append out "/lib/libmudflap.la")
                   (("libdir=.*$")
                    "libdir=$GUIX_GCC_4_7_INSTALL_DIRECTORY'/lib'"))
                 (substitute* (string-append out "/lib/libmudflapth.la")
                   (("libdir=.*$")
                    "libdir=$GUIX_GCC_4_7_INSTALL_DIRECTORY'/lib'"))
                 (substitute* (string-append out "/lib/libquadmath.la")
                   (("libdir=.*$")
                    "libdir=$GUIX_GCC_4_7_INSTALL_DIRECTORY'/lib'"))
                 (substitute* (string-append out "/lib/libssp.la")
                   (("libdir=.*$")
                    "libdir=$GUIX_GCC_4_7_INSTALL_DIRECTORY'/lib'"))
                 (substitute* (string-append out "/lib/libssp_nonshared.la")
                   (("libdir=.*$")
                    "libdir=$GUIX_GCC_4_7_INSTALL_DIRECTORY'/lib'"))
                 (substitute* (string-append out "/lib/libstdc++.la")
                   (("libdir=.*$")
                    "libdir=$GUIX_GCC_4_7_INSTALL_DIRECTORY'/lib'"))
                 (substitute* (string-append out "/lib/libsupc++.la")
                   (("libdir=.*$")
                    "libdir=$GUIX_GCC_4_7_INSTALL_DIRECTORY'/lib'"))
                 (substitute* (string-append out "/lib/libstdc++.so.6.0.17-gdb.py")
                   (("pythondir = .*$")
                    "pythondir = os.environ['GUIX_GCC_4_7_INSTALL_DIRECTORY'] +'/share/gcc-4.7.4/python'"))
                 (substitute* (string-append out "/lib/libstdc++.so.6.0.17-gdb.py")
                   (("libdir = .*$")
                    "libdir = os.environ['GUIX_GCC_4_7_INSTALL_DIRECTORY'] + '/lib'"))
                 (substitute* (string-append out "/libexec/gcc/x86_64-unknown-linux-gnu/4.7.4/install-tools/mkheaders")
                   (("prefix=/gnu/store/.*$")
                    "prefix=$GUIX_GCC_4_7_INSTALL_DIRECTORY")))))))))))
